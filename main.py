...
def calc_to_save(total_cost, saving_plan, travel_date, saved_amount):
    ...
    days_in_month = 30
    days_in_year = 365
    total_cost = int(total_cost) - saved_amount
...
def edit_trip(email, trip_name, changing_param, new_value):
    ...
    if trip_name not in users[email][2]: 
    ...
    #date
    trip_name += email
    if changing_param == "date":
        #change date in trip DS only
        splitted_value = new_value.split('-')
        new_value = datetime(int(splitted_value[0]), int(splitted_value[1]), int(splitted_value[2]))
        ...
        return "Successfully updated date now, you will have to save %.2f %s" % (trips[trip_name][5], trips[trip_name][2])
    #saving_plan
    elif changing_param == "saving_plan":
        #change saving plan in trip DS only 
        trips[trip_name][2] = new_value
        trips[trip_name][5] = calc_to_save(trips[trip_name][1], trips[trip_name][2], trips[trip_name][3], trips[trip_name][4])
        return "Successfully updated saving plan now, you will have to save %.2f %s" % (trips[trip_name][5], trips[trip_name][2])
    #cost
    elif changing_param == "cost":
        trips[trip_name][1] = new_value
        trips[trip_name][5] = calc_to_save(trips[trip_name][1], trips[trip_name][2], trips[trip_name][3], trips[trip_name][4])
        return "Successfully updated cost of trip now, you will have to save %.2f %s" % (trips[trip_name][5], trips[trip_name][2])
    #amount saved
    elif changing_param == "amount_saved":
        trips[trip_name][4] = int(new_value)
        trips[trip_name][5] = calc_to_save(trips[trip_name][1], trips[trip_name][2], trips[trip_name][3], trips[trip_name][4])
        return "Successfully updated you amount saved until now, you wil have to save  %.2f %s" % (trips[trip_name][5], trips[trip_name][2])
